...
"""Test case for caching the contents files in a leveldb database."""
...
class Path2Package(Mapping[bytes, bytes]):
    ...
    def __init__(self, database_path: pathlib.Path) -> None:
        ...
        self.database = plyvel.DB(str(database_path), create_if_missing=True)
    ...
    def __iter__(self) -> Iterator[bytes]:
        with self.database.iterator(include_value=False) as iterator:
            yield from iterator

    def __len__(self) -> int:
        return functools.reduce(lambda i, _: i + 1, iter(self), 0)
    ...
    def update_from_contents_file(self, contents_filename: str, dist: str) -> None:
        ...
        with gzip.open(contents_filename, "rb") as contents:
            ...
            with self.database.write_batch() as wb:
...
def main() -> None:
    """Entry point."""
    args, mapping_db, content_filenames = parse_args_and_common_main(".leveldb")
    file2pkg = _get_file2pkg_mapping(mapping_db, args.release, content_filenames)
    run_access_tests_binary(file2pkg, args)
    memdbg("end")


class TestPath2PackageLevelDB(unittest.TestCase):
...
"""
        ...
        with tempfile.TemporaryDirectory(suffix=self.SUFFIX) as db_path:
            file2pkg = self._Path2PackageClass(pathlib.Path(db_path))
...
"""
    ...
    def test_path2package(self) -> None:
        ...
        with tempfile.TemporaryDirectory(suffix=self.SUFFIX) as db_path:
            path2package = self._Path2PackageClass(pathlib.Path(db_path))
