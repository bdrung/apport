...
Use normal form:
* store the package names in a separate packages table
* Split the directory from the filename and store them in separate tables

Result for jammy on desktop:

```
Creation duration: 56.278 s
Entries in packages table: 72,036
Entries in directories table: 1,007,439
Total entries: 8,037,544
Access duration: 1.654 ms
Total duration: 56.505 s
```

Disqualified because the directories table is too big to be cached in memory.
"""
...
class TestPath2PackageSplitPath(TestPath2Package):
