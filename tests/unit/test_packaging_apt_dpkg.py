        ...
        with tempfile.TemporaryDirectory() as workdir:
            ...
            for truncate_length in (2, 32):
                with self.subTest(truncate_length=truncate_length):
                    ...
                    mapping_path.write_bytes(saved_mapping)
                    self.assertGreater(mapping_path.stat().st_size, truncate_length)
                    with mapping_path.open("a") as mapping_file:
                        mapping_file.truncate(truncate_length)
...
class TestPath2Package(unittest.TestCase):
    """Unit tests for apport.packaging_impl.apt_dpkg._Path2Package class."""
